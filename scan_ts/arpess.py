...
class arpess(Optimizer):
    ...
    def restart_tighten_scan(self):
        if self.adaptive_threshold is not None:
            ...
            print('lowering convergence criterion for constrained optimization to = ',
                  self.fmax * self.fixed_conv_ratio)
        else:
    ...
    def step(self):
        ...
        if len(xx) > 1:
            ...
            if max3pos is None or abs(xx[maxpos] - xx[max2pos]) < abs(xx[maxpos] - xx[max3pos]):
            ...
            if graddiff_hess < 0.0:
                if avg_hess < 0.0:
                    ...
                    if graddiff_hess < min_hess:
                        print(
                            'limit hessian to 10*average hessian from all points since local hessian from two points is too different.',
                            graddiff_hess, avg_hess)
                        graddiff_hess = min_hess
                    if graddiff_hess > max_hess:
                        print(
                            'limit hessian to 0.1*average hessian from all points since local hessian from two points is too different.',
                            graddiff_hess, avg_hess)
        ...
        if len(xx) > 2 and max3pos is not None and self.do_cubic is not None:
            ...
            if newton_statpoint is not None:
    ...
    def setup_starting_structure(self, last_xyz):
        ...
        for ordered in np.argsort([abs(ii - self.a) for ii in a_s]):
            ...
            if self.linear_interpol and len(a_temp) == 2:
                print('starting structure for a = ', self.a,
                      'is generated by interpolation of cartesian coordiates of a1,a2 = ', a_temp[0], a_temp[1])
                old_xyz = xyz_temp[0]
